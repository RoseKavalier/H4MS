...
BOOL Plugin::Hook(_ptr_ address, H4NakedFunction function, _dword_ bytes_to_write /*= 5*/)
{
	return h4::H4Patcher::NakedHook(address, function, bytes_to_write);
}
...
{
	return h4::H4Patcher::BytePatch(address, value);
}

BOOL Plugin::WordPatch(_ptr_ address, _word_ value)
{
	return h4::H4Patcher::WordPatch(address, value);
}

BOOL Plugin::DwordPatch(_ptr_ address, _dword_ value)
{
	return h4::H4Patcher::DwordPatch(address, value);
}
...
VOID Plugin::ShowError(LPCSTR plugin_name, Status status)
{
	...
	h4::H4Error::ShowError(message, plugin_name);
}

BOOL Plugin::isCall(_ptr_ address)
{
	return *reinterpret_cast<_byte_*>(address) == h4::H4Patcher::CALL
		|| *reinterpret_cast<_word_*>(address) == h4::H4Patcher::CALL_DWORD;
}