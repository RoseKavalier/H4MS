...
public:
	...
	*/
	...
	/**
	 * @brief Replace a byte in memory, using lightweight h4::H4Patcher methods.
	 *
	...
	/**
	 * @brief Replace a word in memory, using lightweight h4::H4Patcher methods.
	 *
	 * @param address Location of the replacement.
	 * @param value The new data to be used.
	 */
	...
	/**
	 * @brief Replace a dword in memory, using lightweight h4::H4Patcher methods.
	 *
	 * @param address Location of the replacement.
	 * @param value The new data to be used.
	 */
	...
	/**
	 * @brief Replaces an array of bytes in memory, using lightweight h4::H4Patcher methods.
	 *
	 * @param address Location of the replacement.
	...
	*/
	...
	/**
	 * @brief Writes the most basic type of hook there is using lightweight h4::H4Patcher methods.
	 ...
	 */
	BOOL Hook(_ptr_ address, H4NakedFunction function, _dword_ total_bytes = 5);
	/**
...
template<typename T>
inline BOOL Plugin::AddressOfPatch(const UINT address, const T& data)
{
	return h4::H4Patcher::AddressOfPatch(address, data);
}

template<typename Type, size_t size>
inline BOOL Plugin::AddressOfPatch(const UINT(&address)[size], const Type& data)
{
	return h4::H4Patcher::AddressOfPatch(address, data);
}

template<typename T>
inline BOOL Plugin::ObjectPatch(T& reference, T data)
{
	return h4::H4Patcher::ObjectPatch(reference, data);
}

template<typename T>
inline BOOL Plugin::Write(_ptr_ address, T value)
{
	return h4::H4Patcher::WriteValue(address, value);
}

template<size_t _Size>
inline BOOL Plugin::HexPatch(_ptr_ address, const _byte_(&values)[_Size])
{
	return h4::H4Patcher::WriteValues(address, values);
}

template<typename T>
inline Variable* Plugin::VarInit(LPCSTR name, T value)
{
	return m_patcher->VarInit(name, _dword_(value));
}

template<typename T>
inline T Plugin::VarGetValue(LPCSTR name, T default_value)
{
